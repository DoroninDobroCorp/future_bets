...
func normalizeFootballTeam(team string) string {
	re := regexp.MustCompile(`\b(FC|FK|SC|NK|U\d+)\b|Wom\.`) // отдельное слово: "FC","FK","Wom." или U затем цифры
	team = re.ReplaceAllString(team, "")
	return normalizeAllName(team)
}

func normalizeTennisLeague(league string) string {
	// Слова (\b) Doubles, qualification, clay, STB, hard, grass, carpet
	// отдельное слово если после букв M или W идёт число
	// отдельное слово из цифр
	// всё что в скобках и сами скобки
	re := regexp.MustCompile(`\b(Doubles|qualification|clay|STB|hard|grass|carpet|[M|W]\d+|\d+)\b|\(.*\)`)
...
func normalizeBasketballsTeam(team string) string {
	team = strings.TrimSuffix(team, "Wom.")
