...
def calculate_roi(
	...
	if second_bookmaker_name == "LOBBET":
		if sport_name == "TENNIS":
			return (competitor_odd / (pinnacle_odd * margin * extra_percent) - 1 - 0.03) * 100 * 0.67
		if market_type == 0:
			return (competitor_odd / (pinnacle_odd * margin * extra_percent) - 1 - 0.03) * 100 * 0.67
		if market_type < 0:
			return (competitor_odd / (pinnacle_odd * margin * extra_percent) - 1 - 0.015) * 100 * 0.75
		return (competitor_odd / (pinnacle_odd * margin * extra_percent) - 1 - 0.03) * 100 * 0.67

	elif second_bookmaker_name == "LADBROKES":
		if sport_name == "TENNIS":
			return (competitor_odd / (pinnacle_odd * margin * extra_percent) - 1 - 0.02) * 100 * 0.75
		if market_type == 0:
			return (competitor_odd / (pinnacle_odd * margin * extra_percent) - 1 - 0.02) * 100 * 0.75
		if market_type < 0:
			return (competitor_odd / (pinnacle_odd * margin * extra_percent) - 1) * 100 * 0.85
		return (competitor_odd / (pinnacle_odd * margin * extra_percent) - 1 - 0.02) * 100 * 0.75
