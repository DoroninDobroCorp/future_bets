...
// СКОПИРУЙТЕ И ЗАМЕНИТЕ ВСЕ СОДЕРЖИМОЕ ФАЙЛА
...
// BookmakerOdds - вспомогательная структура для хранения пары "букмекер + цена".
...
// Вспомогательная функция для нормализации числовых ключей (тоталы, форы)
func normalizeOutcomeKey(key string) string {
	value, err := strconv.ParseFloat(key, 64)
	if err != nil {
		return key // Если не число, возвращаем как есть
	}
	return fmt.Sprintf("%.1f", value)
}

// extractAllOutcomes извлекает все доступные рынки из объекта GameData.
					...
					for outcomeStr, prices := range outcomesAcrossGroup {
						...
						if len(prices) >= 3 {
							...
							p.logger.Debug().
								Str("pinnacleMatchId", group.ID).
								Str("outcome", outcomeStr).
								Msgf("[GROUP DEBUG] Found 2 prices: [%s]", finalPricesStr)
						}
					}
				}
