...
import styles from "./OnlineLeaguesPageComponent.module.css";
import $sport from "../../stores/SportStore";
import $bookmaker from "../../stores/BookmakerStore";
import $onlineLeague from "../../stores/OnlineLeaguesStore";
...
import { OnlineLeagueStore } from "@/stores/OnlineLeaguesStore";

export const OnlineLeaguesPageComponent = ({ }): JSX.Element => {
	const bookmakers = useUnit($bookmaker)
	const sports = useUnit($sport)
	const unMatchLeague = useUnit($onlineLeague)
	...
	const addLeaguePairs = async () => {
		if (selectedLeague1 && selectedLeague1 != "" && selectedLeague2 && selectedLeague2 != "") {
			const status = await OnlineLeagueStore.addLeaguePair(selectedLeague1, selectedLeague2)
			if (status == 200) {
	...
	useEffect(() => {
		const fetchUnMatchLeagues = async () => {
			if (selectDropdowns) {
				const [, status] = await OnlineLeagueStore.getLeagues(selectDropdowns.sport, selectDropdowns.bookmaker1, selectDropdowns.bookmaker2)
				if (status == 200) {
	...
	return (
		<>
			<div className={styles.wrapper}>
				<Header header="Создание онлайн лиг" />
