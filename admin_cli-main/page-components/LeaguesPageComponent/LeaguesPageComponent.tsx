...
import styles from "./LeaguesPageComponent.module.css";
import $sport from "../../stores/SportStore";
import $bookmaker from "../../stores/BookmakerStore";
import $unMatchLeague from "../../stores/UnMatchLeagueStore";
...
import { UnMatchLeagueStore } from "@/stores/UnMatchLeagueStore";

export const LeaguesPageComponent = ({ }): JSX.Element => {
	const bookmakers = useUnit($bookmaker)
	const sports = useUnit($sport)
	const unMatchLeague = useUnit($unMatchLeague)
	...
	const addLeaguePairs = async () => {
		if (selectedLeague1 && selectedLeague1 != "" && selectedLeague2 && selectedLeague2 != "") {
			const status = await UnMatchLeagueStore.addLeaguePair(selectedLeague1, selectedLeague2)
			if (status == 200) {
	...
	useEffect(() => {
		const fetchUnMatchLeagues = async () => {
			if (selectDropdowns) {
				const [, status] = await UnMatchLeagueStore.getLeagues(selectDropdowns.sport, selectDropdowns.bookmaker1, selectDropdowns.bookmaker2)
				if (status == 200) {
	...
	return (
		<>
			<div className={styles.wrapper}>
				<Header header="Создание пары лиг" />
