...
import styles from "./AllLeaguesPageComponent.module.css";
import $sport from "../../stores/SportStore";
import $bookmaker from "../../stores/BookmakerStore";
import $allLeagues, { AllLeagueStore } from "../../stores/AllLeaguesStore";
...
export const AllLeaguesPageComponent = ({ }): JSX.Element => {
    const bookmakers = useUnit($bookmaker)
    const sports = useUnit($sport)
    const unMatchLeague = useUnit($allLeagues)
    ...
    const addLeaguePairs = async () => {
        if (selectedLeague1 && selectedLeague1 != "" && selectedLeague2 && selectedLeague2 != "") {
            const status = await AllLeagueStore.addLeaguePair(selectedLeague1, selectedLeague2)
            if (status == 200) {
    ...
    useEffect(() => {
        const fetchUnMatchLeagues = async () => {
            if (selectDropdowns) {
                const [, status] = await AllLeagueStore.getLeagues(selectDropdowns.sport, selectDropdowns.bookmaker1, selectDropdowns.bookmaker2)
                if (status == 200) {
    ...
    return (
        <>
            <div className={styles.wrapper}>
                <Header header="Создание пары лиг (все!)" />
