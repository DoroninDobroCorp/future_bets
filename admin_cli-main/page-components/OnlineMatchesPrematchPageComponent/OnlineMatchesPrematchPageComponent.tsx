...
import styles from "./OnlineMatchesPrematchPageComponent.module.css";
...
import $onlineUnMatchTeamPrematch from "../../stores/OnlineUnMatchTeamPrematchStore"
...
import { OnlineUnMatchTeamPrematchStore } from "../../stores/OnlineUnMatchTeamPrematchStore";
import { toast } from "react-toastify";
import { ListMatchTeam } from "@/components/List/ListMatchTeam";
import { ITeam } from "@/interfaces/Team.interface";

export const OnlineMatchesPrematchPageComponent = ({ }): JSX.Element => {
    const bookmakers = useUnit($bookmaker)
    const sports = useUnit($sport)
    const unMatchTeam = useUnit($onlineUnMatchTeamPrematch)
    ...
    useEffect(() => {
        const fetchUnMatchTeams = async () => {
            if (selectDropdowns) {
                const [, status] = await OnlineUnMatchTeamPrematchStore.getUnMatchTeams(selectDropdowns.sport, selectDropdowns.bookmaker1, selectDropdowns.bookmaker2)
                if (status == 200) {
    ...
    const addTeamPairs = async () => {
        if (selectedTeam1 && selectedTeam1 != "" && selectedTeam2 && selectedTeam2 != "") {
            const status = await OnlineUnMatchTeamPrematchStore.addLeaguePair(selectedTeam1, selectedTeam2)
            if (status == 200) {
    ...
    return (
        <>
            <div className={styles.wrapper}>
                <Header header="Создание онлайн команд PREMATCH" />
