...
import styles from "./OnlineMatchesPageComponent.module.css";
...
import $onlineUnMatchTeam from "../../stores/OnlineUnMatchTeamStore"
...
import { OnlineUnMatchTeamStore } from "../../stores/OnlineUnMatchTeamStore";
import { toast } from "react-toastify";
import { ListMatchTeam } from "@/components/List/ListMatchTeam";
import { ITeam } from "@/interfaces/Team.interface";

export const OnlineMatchesPageComponent = ({ }): JSX.Element => {
    const bookmakers = useUnit($bookmaker)
    const sports = useUnit($sport)
    const unMatchTeam = useUnit($onlineUnMatchTeam)
    ...
    useEffect(() => {
        const fetchUnMatchTeams = async () => {
            if (selectDropdowns) {
                const [, status] = await OnlineUnMatchTeamStore.getUnMatchTeams(selectDropdowns.sport, selectDropdowns.bookmaker1, selectDropdowns.bookmaker2)
                if (status == 200) {
    ...
    const addTeamPairs = async () => {
        if (selectedTeam1 && selectedTeam1 != "" && selectedTeam2 && selectedTeam2 != "") {
            const status = await OnlineUnMatchTeamStore.addLeaguePair(selectedTeam1, selectedTeam2)
            if (status == 200) {
    ...
    return (
        <>
            <div className={styles.wrapper}>
                <Header header="Создание онлайн команд" />
